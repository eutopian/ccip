...
contract EVM2EVMMultiOnRamp is IEVM2AnyOnRampClient, ILinkAvailable, AggregateRateLimiter, ITypeAndVersion {
  ...
  string public constant override typeAndVersion = "EVM2EVMMultiOnRamp 1.2.0";
  ...
  mapping(uint64 destChainId => bool) internal s_enabledDestChains;
  ...
  /// real transaction.
  mapping(uint64 destChainSelector => uint64 seqNum) internal s_sequenceNumber;
  ...
  function addDestChain(uint64 destChainSelector) external onlyOwner {
    s_enabledDestChains[destChainSelector] = true;
  }
  ...
  function getExpectedNextSequenceNumber(uint64 destChainSelector) external view returns (uint64) {
    return s_sequenceNumber[destChainSelector] + 1;
  }

  function getSenderNonce(uint64 destChainSelector, address sender) external view returns (uint64) {
    uint256 senderNonce = s_senderNonce[destChainSelector][sender];
  ...
  function forwardFromRouter(
    ...
    if (!s_enabledDestChains[destChainSelector]) revert InvalidChainSelector(destChainSelector);

    MessageArgs memory args = _getMessageArgs(message);
    ...
    if (s_senderNonce[destChainSelector][originalSender] == 0 && i_prevOnRamp != address(0)) {
      ...
      s_senderNonce[destChainSelector][originalSender] = IEVM2AnyOnRamp(i_prevOnRamp).getSenderNonce(originalSender);
    }
    ...
    Internal.EVM2EVMMessage memory newMessage = Internal.EVM2EVMMessage({
      ...
      sequenceNumber: ++s_sequenceNumber[destChainSelector],
      gasLimit: args.gasLimit,
      strict: false,
      nonce: ++s_senderNonce[destChainSelector][originalSender],
      feeToken: message.feeToken,
      feeTokenAmount: feeTokenAmount,
      data: message.data,
      tokenAmounts: message.tokenAmounts,
      sourceTokenData: new bytes[](args.numberOfTokens), // will be filled in later
      messageId: ""
    });

    // Lock the tokens as last step. TokenPools may not always be trusted.
    // There should be no state changes after external call to TokenPools.
    for (uint256 i = 0; i < args.numberOfTokens; ++i) {
    ...
    newMessage.messageId = Internal._hash(
      newMessage,
      // calculate metadatahash on the fly as there can be multiple dest chain selectors
  ...
  function _getMessageArgs(Client.EVM2AnyMessage calldata message) internal returns (MessageArgs memory) {
    MessageArgs memory args;
    ...
    args.receiver = address(uint160(decodedReceiver));

    args.gasLimit = _fromBytes(message.extraArgs);
    args.numberOfTokens = message.tokenAmounts.length;

    // Validate the message with various checks
    _validateMessage(message.data.length, args.gasLimit, args.numberOfTokens);

    // Only check token value if there are tokens
    if (args.numberOfTokens > 0) {
      for (uint256 i = 0; i < args.numberOfTokens; ++i) {
  ...
  function _fromBytes(bytes calldata extraArgs) internal view returns (uint256 gasLimit) {
    if (extraArgs.length == 0) {
      return i_defaultTxGasLimit;
    }
  ...
  function getFee(
    ...
    uint256 gasLimit = _fromBytes(message.extraArgs);
    // Validate the message with various checks
